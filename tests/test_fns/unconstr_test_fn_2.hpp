...
double unconstr_test_fn_2(const arma::vec& vals_inp, arma::vec* grad, void* opt_data);

double
unconstr_test_fn_2(const arma::vec& vals_inp, arma::vec* grad, void* opt_data)
{
    ...
    //
    if (grad) {
        (*grad)(0) = -400*(x_2 - std::pow(x_1,2))*x_1 - 2*(1-x_1);
        (*grad)(1) = 200*(x_2 - std::pow(x_1,2));
    }
    //
    return obj_val;
}
